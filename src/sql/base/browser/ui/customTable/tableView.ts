...
import { IDelegate, IRenderer, IListMouseEvent, IListTouchEvent, IListGestureEvent } from './table';
...
const DefaultOptions: ITableViewOptions = {
	useShadows: true,
...
export class TableView<T> implements ISpliceable<T>, IDisposable {
	...
	constructor(
		...
		options: ITableViewOptions = DefaultOptions
	) {
		this.items = [];
		this.itemId = 0;
		...
		this._domNode.className = 'monaco-table';
